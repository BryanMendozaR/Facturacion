...
{
    internal class FiltrarClienteHandler : IRequestHandler<FiltrarCliente, IList<ClienteModelo>>
    {
        ...
        public async Task<IList<ClienteModelo>> Handle(FiltrarCliente request, CancellationToken cancellationToken)
        {
            if (string.IsNullOrEmpty(request.Nombre.Trim()))
                return await _clienteRepositorio.ConsultarAsync();

            return await _clienteRepositorio.ConsultarPorNombreAsync(request.Nombre);
        }
    }
}
